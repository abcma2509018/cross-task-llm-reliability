...
def normalize_gsm8k_number(value):
    ...
    boxed = regex.findall(r"\\boxed\{([^{}]+)\}", text)
    if boxed:
        text = boxed[-1]
    nums = regex.findall(r"-?\d+(?:\.\d+)?", text)
    if not nums:
        return None
    num = nums[-1]
    if "." in num:
...
def has_gsm8k_answer(answers, text):
    pred = normalize_gsm8k_number(text)
    if pred is None:
        return 0
    for answer in answers:
        ref = normalize_gsm8k_number(answer)
