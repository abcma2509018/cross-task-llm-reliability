import time
import os
from .utils import deal_answer, deal_judge, deal_post, str2paras, deal_judge_new, has_answer, has_gsm8k_answer
...
QWEN_MODEL_NAMES = ['qwen2-7b-instruct', 'qwen2.5-7b-instruct']
...
class Generater:
    def __init__(self, args):
        ...
        if '7b' in self.args.model_name or '8b' in self.args.model_name:
            self.model = AutoModelForCausalLM.from_pretrained(args.model_path, **load_kwargs)
        else:
            self.model = AutoModelForCausalLM.from_pretrained(args.model_path, **load_kwargs).half()
    ...
    def get_res(self):
        ...
        for batch in tqdm(self.dataloader):
            ...
            outs = self.model.generate(input_ids, attention_mask=attn_mask, max_new_tokens=self.args.max_new_tokens, 
                                       ...
                                       pad_token_id=self.tokenizer.pad_token_id, top_p=1.0, temperature=1, do_sample=False)
            if self.args.task == 'mmlu' or self.args.task == 'tq':
    ...
    def process_res_multi_choice(self, outs, inputs):
        ...
        if self.args.model_name in ['llama3-8b-instruct'] + QWEN_MODEL_NAMES:
                ...
                if 'qa' in self.args.type:
                    ...
                    if self.args.task == 'mmlu' or self.args.task == 'tq':
                        ...
                        if self.args.task == 'gsm8k':
                            res_sample['has_answer'] = has_gsm8k_answer(all_data[idx]['reference'], res_sample['Res'])
                        else:
    ...
    def get_choice_idx(self, outs, inputs, end_idx):
        ...
        if self.args.model_name in ['llama2-7b-chat', 'llama2-13b-chat']:
            ...
            #['_A'], ['(A', ')']
        elif self.args.model_name in ['llama3-8b-instruct'] + QWEN_MODEL_NAMES:
